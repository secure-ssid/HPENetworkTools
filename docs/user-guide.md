...
`?state=` param; click again to clear.
...
(unique newline-joined inventory serials), **Copy selection link** (`?ids=` of
...
**Export selected**, **Copy BSSIDs**, and **Copy selection link**
(`?bssids=` + `section=rogues`; clearable chip — Loop 193). Live mode also offers
...
inventing rows; filtered empties offer **Clear filters**.
...
- **Keyboard rows.** On Devices, Alerts, Clients, Auth events, Sites, Topology nodes,
  ...
  overlay (Topology / Licences / UXI keyboard help is Loop 192; ClearPass /
  ...
  help is Loop 202). A non-empty selection raises a bulk bar with **Export selected**
  ...
  Overview Needs-you-now + Sites preview bulk is Loop 190; Central webhooks list bulk
  is Loop 190; Mist estate + SiteDetail rogue APs bulk and Mist org audit log bulk
  are Loop 193; GreenLake role grants + locations bulk is Loop 196).
...
- **Copy view link.** Most list and detail headers offer **Copy view link**,
  ...
  **Devices** does the same on the unified table: selection raises **Export
  ...
  serial are skipped). Header **LIVE** stamps pure live and blend feeds alike
  ...
  **Copy selection link** (`?macs=` of unique inventory/endpoint MACs; clearable
  chip while active — Loop 160) beside **Copy MACs**. **Sites** multi-select raises
  ...
  **Clear filters**; header **LIVE** stamps pure live
  ...
  `?ids=` offers **Clear selection filter** (Loop 208). **Licences** multi-select
  raises **Export selected**, **Copy SKUs**, **Copy selection link** (`?skus=`;
  clearable chip — Loop 172), and **Clear** (Loop 162) plus keyboard shortcuts
  help; live empty subscriptions offer **Connected systems** (Loop 192); header
  **LIVE** stamps pure live and licenses blend alike (Loop 166). **Alerts**
  ...
  newline-joined published sensor serials — Loop 169) and **Copy selection link**
  (`?ids=` of marked sensor ids; clearable chip — Loop 175) beside **Export selected**,
  plus keyboard shortcuts help on the sensors table (Loop 192).
  **Systems** header **LIVE** badge follows pure live and systems blend beside
  ...
  ids), **Copy selection link** (`?ids=` of marked plane ids; clearable chip —
  Loop 189; drawer open stays on `?plane=`), and **Clear**. **Site detail** header **LIVE**
  ...
  **Inventory** explorer header **LIVE** stamps a non-demo systems registry
  ...
  (unique newline-joined ticket ids — Loop 171), **Copy selection link**
  ...
  endpoints multi-select raises **Export selected**, **Copy MACs**, **Copy
  ...
  **Copy selection link** (`?ids=` of marked site ids with `section=sites`;
  ...
  selected**, **Copy serials**, **Copy selection link** (`?names=` of marked
  device names; clearable chip — Loop 181), and **Clear** (Loop 174); selection-empty
  `?names=` offers **Clear selection filter** (Loop 208). **Overview** header
  **LIVE** stamps pure live (Loop 168 — blend keeps per-section LIVE/DEMO
  badges). Empty Overview sections offer operator CTAs (Open Alerts / Connected
  systems / Inventory / Open Configure / Clear health filter — Loop 189).
  ...
  **Remove selected**, and **Clear** beside the existing Clear-all control
  ...
  multi-select raises **Export selected**, **Copy emails**, **Copy selection
  link** (`?ids=` of marked member ids; clearable chip — Loop 177), and **Clear**
  (Loop 172) plus keyboard shortcuts help on the members table; filtered empties
  offer **Clear filters** (Loop 195); role grants multi-select raises **Export
  selected**, **Copy principals**, **Copy selection link** (`?roleIds=` with
  `section=roles`; clearable chip — Loop 196), and **Clear**; locations multi-select
  ...
  **Copy rules** (unique newline-joined rule ids — Loop 172), **Copy selection
  link** (`?rules=` of unique rule ids; clearable chip — Loop 177), and **Clear**
  (Loop 165). **Central** firmware multi-select raises **Export selected**,
  **Copy serials**, **Copy selection link** (`?serials=` with `section=firmware`;
  clearable chip — Loop 181), and **Clear** (Loop 177).   **Central** WLANs multi-select raises **Export selected**, **Copy names** (unique newline-joined SSID names), **Copy selection link** (`?names=` with `section=wlans`; clearable chip — Loop 183), and **Clear**; sites/WLANs selection-empty deep links offer **Clear selection filter** (Loop 207). **Configure** queue multi-select adds **Export selected**, **Copy IDs** (unique newline-joined broker ids), and **Copy selection link** (`?ids=` of queue row keys with `section=queue`; clearable chip — Loop 183) beside Approve/Reject plus keyboard shortcuts help on the queue table (Loop 195); port filter empties offer **Clear filters** and queue selection-empty offers **Clear selection filter** (Loop 205). **Recommendations** selection-empty deep links offer **Clear selection filter** (Loop 205). **Systems** SSE inventory multi-select raises **Export selected**, **Copy IDs**, **Copy selection link** (`?sseIds=` with kind/q; clearable chip — Loop 183), and **Clear** plus keyboard shortcuts help and filtered empty **Clear selection filter** / **Clear search** (Loop 201). **Mist** firmware
  multi-select raises **Export selected**, **Copy serials** (unique
  newline-joined inventory serials — Central firmware pattern), **Copy selection
  link** (`?serials=` with `section=devices`; clearable chip — Loop 184), and
  **Clear** (Loop 180). **Mist** WLANs multi-select raises **Export selected**,
  ...
  **Copy selection link** (`?names=` with `section=wlans`; clearable chip —
  ...
  enabled (Loop 204). **Mist** licence usage multi-select raises **Export
  selected**, **Copy site ids** (unique newline-joined site ids), **Copy
  selection link** (`?siteIds=` with `section=licenses`; clearable chip —
  Loop 187), and **Clear**. **Device detail** clients multi-select raises **Export
  selected**, **Copy MACs** (unique newline-joined session MACs), **Copy
  selection link** (`?macs=`; clearable chip — Loop 184), and **Clear**
  (Loop 180). **Device detail** ports (class-block and live interfaces)
  multi-select raises **Export selected**, **Copy ports** (unique newline-joined
  port names), **Copy selection link** (`?ports=`; clearable chip — Loop 187),
  and **Clear**; ports selection-empty offers **Clear selection filter**
  (Loop 207). **Inventory** search multi-select raises **Export selected**,
  **Copy serials**, **Copy selection link** (`?ids=` of marked node ids;
  clearable chip — Loop 184), and **Clear** (Loop 180 — Enter still opens the
  focused hit) plus keyboard shortcuts help on the explorer header (Loop 198).
...
the shortcut map — Loop 201). A search that matches nothing offers **Clear
search**.
...
**Export selected**, **Copy admins**, and **Copy selection link**
