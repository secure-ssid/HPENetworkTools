 ...
 *   subscriptions  /subscriptions/v1/subscriptions?workspace_id=…
 ...
 * No pagination: subscription lists are small (one row per licence key); the
 * first page is the whole truth at this scale.
 *
 * Mapping decisions:
 ...
 *   - assigned: direct count fields first, then an assignments array's length
 *     ("from assignments if present").
...
export function subStatusFor(
  ...
  if (/retir|expired|cancel|end[- ]?of|terminat/.test(s)) return { status: 'retiring', tone: 'danger' };
...
/** assigned count: direct fields first, then an assignments array's length. */
function assignedCount(r: Record<string, unknown>): number | null {
  const direct = num(r.assigned ?? r.assigned_count ?? r.assigned_quantity ?? r.used ?? r.used_quantity ?? r.consumed);
...
function termString(r: Record<string, unknown>): string {
  const direct = str(r.term ?? r.subscription_term ?? r.term_description);
  if (direct) return direct;
  const months = num(r.term_months ?? r.duration_months);
  if (months !== null && months > 0) {
    return months % 12 === 0 ? `${months / 12} yr subscription` : `${months} mo subscription`;
  }
  const start = parseTimestamp(r.start_date ?? r.starts ?? r.valid_from);
  const end = parseTimestamp(r.end_date ?? r.expires ?? r.expiration_date ?? r.expiry_date);
...
export function mapGreenLakeSubscription(raw: unknown, nowMs: number = Date.now()): GreenLakeSubscriptionRow | null {
  ...
  const qty = num(r.quantity ?? r.total_quantity ?? r.license_count ?? r.qty ?? r.seats);
  const assigned = assignedCount(r);
  const expiresAtMs = parseTimestamp(r.expires ?? r.expiration_date ?? r.expiry_date ?? r.end_date ?? r.valid_until);
  const daysLeft = expiresAtMs !== null ? Math.floor((expiresAtMs - nowMs) / DAY_MS) : null;
  const { status, tone } = subStatusFor(str(r.status ?? r.state), assigned, daysLeft);
  return {
    name,
    sku: str(r.sku) ?? str(r.part_number) ?? str(r.subscription_key) ?? str(r.subscription_id) ?? str(r.id) ?? '—',
    plane: 'GREENLAKE',
    planeTone: 'accent',
    term: termString(r),
...
export class GreenLakeAdapter implements PlaneAdapter {
  ...
  constructor(
    ...
    this.candidates = [
      `/subscriptions/v1/subscriptions?workspace_id=${ws}`,
      '/subscription-manager/v1/subscriptions',
      '/devices/v1/subscriptions',
    ];
  ...
  async pull(): Promise<PlanePull> {
    let rows: unknown[];
    try {
      rows = await this.fetchSubscriptions();
    } catch (err) {
  ...
  private async fetchSubscriptions(): Promise<unknown[]> {
    const resolved = this.resolvedPath;
    const candidates = resolved ? [resolved, ...this.candidates.filter((c) => c !== resolved)] : this.candidates;
    for (const path of candidates) {
      const res = await this.authedGet(path);
      if (res.status === 404) continue; // workspace/release variance — try the next API
